...
max_points_on_line = 0  # инициализируем максимальное количество точек на прямой
max_line_points = []  # инициализируем список точек на прямой с максимальным количеством точек

for i in range(2, len(points) + 1):
    for comb in generate_combinations(points, i):
        line_points = list(comb)
        if all(point[0] % 2 != 0 and point[1] % 2 != 0 for point in line_points):  # ограничение на нечетные координаты
